import Vue from 'vue';
import pathUtil from 'web/common/utils/pathUtils';
import 'web/resources/lib/bootstrap-sweetalert/lib/sweet-alert.min';
...
export default {
	data(){
		return {
			...
			isLogining:false,
			submitClass:'btn btn-danger btn-block'
		}
	},
	methods:{
		login(){
			if(!this.obj.userName || !this.obj.password){
				...
				this.obj.password = Encryptor(this.obj.password);
				this.$http.jsonp(pathUtil.getHttpPath()+"/auth/login",{params:this.obj}).then(function(ret){
					console.log(ret);
					alert(ret);
				});
			}
