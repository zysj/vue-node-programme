...
//                }
    ...
    var Upload = {
        ...
        config: function (sel) {
            ...
            url=url + (url.indexOf("?") == -1 ? "?" : "&") + "savePath="+me.editor.options.savePath;//本地化：url携带文件上传文件夹名称参数
            url=me.editor.options.fileType && (url + (url.indexOf("?") == -1 ? "?" : "&") + "fileType="+me.editor.options.fileType);//本地化：url携带文件上传文件夹类型参数
*/            $("form", $(sel, me.dialog)).attr("action", me.initUrl());
        ...
        initUrl:function(){
        	var me = this;
        	var param = me.editor.getOpt('uploadParams');
        	me.editor.id && (param['editorid'] = me.editor.id);
        	var url = me.editor.getOpt('imageUrl');
        	if(param == null || param === undefined)return url;
        	var paramUtil = me.editor.getOpt('paramUtil');
        	var query = paramUtil(param);
        	return url + "?" + query;
        },
        uploadComplete: function(r){
            ...
            try{
                var json = eval('('+r+')');
                //本地化：处理json对象（注：可以合并在uploadComplete函数中）
                json.url = json.bean.id;
                json.state = json.type.toUpperCase();
        ...
        //本地化：运行filter数组中的函数
        filterHandle:function(file){
        	var me = this;
        	var error;
        	var filters = me.editor.options.imgFilters;
        	var isOk = filters.every(function(fn){
        		var ret = fn(file,me);
        		if(!error&&ret.msg ){
        			error = ret.msg;
        			swal({
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
            	if(this.files.length&&!me.filterHandle(this.files[0])){
            		return false;
            	}
            	if ( !this.parentNode ) {
                    return;
                ...
                me.dialog.find('.edui-image-content').on('drop',function (e) {
                    ...
                    $.each(fileList, function (i, f) {
                        if (/^image/.test(f.type)) {
                            ...
                            //本地化：初始化上传地址
                            var url = me.initUrl();
                            url=url + (url.indexOf("?") == -1 ? "?" : "&")+"type=ajax";
                            
                            xhr.open("post", url, true);
                            xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
                            xhr.setRequestHeader("Accept","application/json, text/plain");//本地化：返回json格式的报文
                            //模拟数据
                            var fd = new FormData();
